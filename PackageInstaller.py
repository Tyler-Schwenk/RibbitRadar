...
def get_remote_version_info(file_id):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logging.error(
                "Failed to fetch remote version info. Status code: %s",
...
def check_and_update_model(
    local_version_path, remote_version_file_id, model_output_dir
):
    """
    ...
    try:
        ...
        if remote_model_info:
            ...
            logging.info(
                f"Remote model version: {remote_model_version}, Local model version: {local_model_version}"
            )

            if local_model_version != remote_model_version:
                logging.info("New model version detected. Downloading...")
                model_output_path = os.path.join(
                    model_output_dir, f"model_{remote_model_version}.pth"
                )
            ...
            logging.warning(
                "Could not fetch remote model version info. Skipping model update."
            )
    except Exception as e:
...
def get_model_info(local_version_path, model_output_dir):
    ...
    model_weights_file = os.path.join(
        model_output_dir, f"model_{local_model_version}.pth"
    )

    if not os.path.exists(model_weights_file):
        logging.warning(
            f"Model weights file not found for version {local_model_version}."
        )
        return None, None
