...
def train(audio_model, train_loader, test_loader, args):
    ...
    print(
        "The learning rate scheduler starts at {:d} epoch with decay rate of {:.3f} every {:d} epochs".format(
    ...
    while epoch < args.n_epochs + 1:
        ...
        middle_ps = [
            stat["precisions"][int(len(stat["precisions"]) / 2)] for stat in stats
        ]
        ...
        torch.save(
            audio_model.state_dict(), "%s/models/audio_model.%d.pth" % (exp_dir, epoch)
        )
        if len(train_loader.dataset) > 2e5:
            torch.save(
