...
def preprocess_audio_pipeline(input_dir, progress_callback):
    ...
    try:
        ...
        audio_preprocessing.split_all_audio_files(input_dir, paths.TEMP_FILE_STORAGE, progress_callback)

        # Resample and convert audio files
        progress_callback("Preprocessing Step 3/5: Converting to mono...", 20, "Converting to mono...")
        audio_preprocessing.stereo_to_mono(paths.TEMP_FILE_STORAGE)

        progress_callback("Preprocessing Step 4/5: Resampling audio...", 50, "Resampling audio...")
        audio_preprocessing.preprocess_audio(input_dir, paths.TEMP_FILE_STORAGE, paths.RESAMPLED_AUDIO_PATH, progress_callback)
